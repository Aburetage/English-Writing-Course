...
(function(){
  ...
  document.addEventListener('touchend',e=>{
    ...
    if(Math.abs(dx)>70&&Math.abs(dy)<50){
      if(dx<0)showMod(cur+1); else showMod(cur-1);
    }
  },{passive:true});
})();
(function(){
...
(function(){
  ...
  $$('.bb-tab').forEach(t=>{
    t.addEventListener('click',e=>{
      ...
      if(k==='map'&&isIndex){e.preventDefault();const el=document.getElementById('roadmap');showMod(modOf(el),'roadmap');return;}
      if(k==='home'&&isIndex){e.preventDefault();showMod(0);scrollTo({top:0,behavior:'smooth'});return;}
